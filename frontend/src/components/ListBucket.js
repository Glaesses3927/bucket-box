...
import { setChosenItem } from '../store/targetSlice';
...
export default function ListBucket({ today }) {
  ...
  let showlist = bucketList.filter(item => 
  ...
  const listbucket = showlist.map(item => 
    <li onClick={() => {dispatch(setChosenItem(item.id));dispatch(setEditting(false));}} className='p-2 flex hover:bg-gray-100 hover:cursor-pointer' key={item.id}>
      <div>{item.title}</div>
      {item.completed===1 &&
